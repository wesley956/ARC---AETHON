...
export function createEmptyMaterialInventory(): MaterialInventory { return { living_ash: 0, ancient_stone: 0, shell_fragment: 0, memory_echo: 0 }; }

const VALID_MATERIAL_IDS: MaterialId[] = ['living_ash', 'ancient_stone', 'shell_fragment', 'memory_echo'];
function isValidMaterialId(id: string): id is MaterialId { return VALID_MATERIAL_IDS.includes(id as MaterialId); }
...
export function normalizeMaterialInventory(input: unknown): MaterialInventory {
  ...
  for (const key of VALID_MATERIAL_IDS) {
    const value = obj[key];
    if (typeof value === 'number' && value >= 0 && Number.isFinite(value)) result[key] = Math.floor(value);
  }
  return result;
}
...
export function addMaterials(existing: MaterialInventory, toAdd: Partial<MaterialInventory>): MaterialInventory {
  ...
  for (const [key, amount] of Object.entries(toAdd)) {
    if (isValidMaterialId(key) && typeof amount === 'number') result[key] = (result[key] || 0) + amount;
  }
  return result;
}
