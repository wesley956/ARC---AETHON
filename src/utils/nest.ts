...
export function normalizeNestData(input: unknown): NestData {
  ...
  if ('slots' in obj && typeof obj.slots === 'object' && obj.slots !== null) {
    ...
    return {
      ...
      style: isValidStyle(obj.style) ? obj.style as NestData['style'] : 'basic',
      slots: { base: isValidSlot(slots.base) ? slots.base as NestSlot : null, comfort: isValidSlot(slots.comfort) ? slots.comfort as NestSlot : null, relic: isValidSlot(slots.relic) ? slots.relic as NestSlot : null },
  ...
  return { ...defaultNest, comfort: typeof obj.comfortLevel === 'number' ? obj.comfortLevel : 0 };
}

function isValidStyle(value: unknown): boolean { return typeof value === 'string' && ['basic', 'warm', 'stone', 'memory'].includes(value); }
function isValidSlot(value: unknown): boolean { if (!value || typeof value !== 'object') return false; const s = value as Record<string, unknown>; return typeof s.id === 'string' && typeof s.name === 'string'; }
...
export function determineNestStyle(slots: NestData['slots']): NestData['style'] {
  if (slots.relic) return 'memory'; if (slots.comfort) return 'warm'; if (slots.base) return 'stone'; return 'basic';
}
