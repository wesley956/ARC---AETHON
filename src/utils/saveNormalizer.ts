...
function createDefaultProfessionProgress(): ProfessionProgress { return { discoveredProfession: null, professionXp: 0, professionLevel: 0, hints: [], tendencies: { guardian: 0, forge: 0, memory: 0 } }; }

function normalizeDragonData(dragon: unknown): DragonData | null {
  ...
  let crystals: CrystalInventory = createDefaultCrystals();
  ...
  let personalityTraits: PersonalityTraits = createDefaultTraits();
  ...
  let professionProgress: ProfessionProgress = createDefaultProfessionProgress();
  if (d.professionProgress && typeof d.professionProgress === 'object') {
    const pp = d.professionProgress as Record<string, unknown>;
    professionProgress = { discoveredProfession: typeof pp.discoveredProfession === 'string' ? pp.discoveredProfession : null, professionXp: typeof pp.professionXp === 'number' ? pp.professionXp : 0, professionLevel: typeof pp.professionLevel === 'number' ? pp.professionLevel : 0, hints: Array.isArray(pp.hints) ? pp.hints : [], tendencies: (pp.tendencies as Record<string, number>) || { guardian: 0, forge: 0, memory: 0 } };
  }
  return {
    dragonName: d.dragonName as string, dragonType: d.dragonType as string, dominantElement: (d.dominantElement as DragonData['dominantElement']) || 'earth',
    vitality: typeof d.vitality === 'number' ? Math.min(1, Math.max(0, d.vitality)) : 0.75, personalityTraits,
    isOnExpedition: d.isOnExpedition === true, expeditionEndTime: typeof d.expeditionEndTime === 'number' ? d.expeditionEndTime : null,
    expeditionZoneId: (d.expeditionZoneId as DragonData['expeditionZoneId']) || null, expeditionLayerId: (d.expeditionLayerId as DragonData['expeditionLayerId']) || null,
    ...
    diaryEntries: Array.isArray(d.diaryEntries) ? d.diaryEntries : [], crystals, materials, nestData, professionProgress,
