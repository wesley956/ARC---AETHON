      ...
      <div className="flex flex-col items-center gap-6 max-w-sm text-center">
        ...
        {validationErrors.length > 0 && (
          <div className="w-full bg-red-900/20 border border-red-700/50 rounded-lg p-4 text-left">
            ...
            <ul className="text-xs text-red-300/80 space-y-1">{validationErrors.map((error, i) => (<li key={i}>• {error}</li>))}</ul>
          </div>
        )}
        <button onClick={clearSave} className="w-full py-3 bg-red-900 hover:bg-red-800 text-red-200 font-semibold rounded-xl border border-red-700">🗑️ [DEV ONLY] Limpar Save e Recomeçar</button>
      </div>
    </Layout>
  );
}
