    ...
    <Layout className="items-center justify-center px-6">
      {phase === 'preparing' && (
        <div className="flex flex-col items-center gap-6 animate-fade-in">
          ...
          <p className="text-lg text-[#e8e8ec] text-center">Algo se move dentro da casca...</p>
        </div>
      )}

      {phase === 'cracking' && (
        <div className="flex flex-col items-center gap-6 animate-fade-in">
          <div className="text-7xl animate-shake" style={{ animationDuration: '0.2s' }}>🥚</div>
          <p className="text-lg text-[#e8e8ec] text-center">A casca estala. Rachaduras surgem.</p>
        </div>
      )}

      {phase === 'pulse' && (
        <div className="flex flex-col items-center gap-6 animate-fade-in">
          <div className="text-7xl animate-glow" style={{ filter: `drop-shadow(0 0 20px ${dominantColor})` }}>🥚</div>
          <p className="text-lg text-[#e8e8ec] text-center">A energia acumulada responde.</p>
        </div>
      )}
      ...
      {phase === 'burst' && (
        <div className="flex flex-col items-center gap-6 animate-fade-in">
          ...
          <p className="text-lg text-[#e8e8ec] text-center">A casca explode em luz.</p>
        </div>
      )}

      {phase === 'reveal' && (
        <div className="flex flex-col items-center gap-6 animate-fade-in max-w-sm">
          <div className="text-8xl animate-float" style={{ filter: `drop-shadow(0 0 30px ${dominantColor})` }}>🐉</div>
          <div className="text-center space-y-2">
            <h2 className="text-xl font-bold text-[#c4b5fd]">{resolvedType.dragonTypeName}</h2>
            <p className="text-sm text-[#e8e8ec] leading-relaxed whitespace-pre-line">{narrativePhrase}</p>
          </div>
          <button
      ...
      {phase === 'naming' && (
        <div className="flex flex-col items-center gap-6 animate-fade-in w-full max-w-sm">
          <div className="text-7xl animate-float" style={{ filter: `drop-shadow(0 0 20px ${dominantColor})` }}>🐉</div>
          <div className="text-center">
            <h2 className="text-xl font-bold text-[#c4b5fd] mb-2">Como você vai chamá-lo?</h2>
            <p className="text-sm text-[#6a6a7a]">Esse nome será para sempre.</p>
          </div>
          <div className="w-full">
            ...
            />
            {nameError && <p className="text-red-400 text-sm mt-2 text-center">{nameError}</p>}
          </div>
          <button
