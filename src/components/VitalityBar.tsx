...
export default function VitalityBar({ vitality, showLabel = true }: VitalityBarProps) {
  ...
  return (
    <div className="rounded-xl p-4" style={{ background: 'linear-gradient(135deg, rgba(18,18,26,0.6) 0%, rgba(18,18,26,0.4) 100%)', border: '1px solid rgba(42,42,58,0.5)' }}>
      {showLabel && (
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2"><span className="text-lg">❤️</span><span className="text-sm font-medium text-[#e8e8ec]">Vitalidade</span></div>
          <span className="text-sm font-bold" style={{ color: colors.from }}>{percentage}%</span>
        </div>
      )}
      <div className="relative w-full h-3 bg-[#1a1a24] rounded-full overflow-hidden">
        <div className="relative h-full rounded-full transition-all duration-700 ease-out" style={{ width: `${percentage}%`, background: `linear-gradient(90deg, ${colors.from} 0%, ${colors.to} 100%)`, boxShadow: `0 0 10px ${colors.glow}` }}>
      ...
      {showLabel && <p className="text-xs text-[#6a6a7a] mt-2 italic">{getMessage()}</p>}
    </div>
  );
}
