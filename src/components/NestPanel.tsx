...
// Mobile-optimized nest management.
// ============================================================

import { useState } from 'react';
import { DragonData, NestSlotType, MaterialId } from '../types/game';
...
export default function NestPanel({ dragon, onUpdate, onNotify }: NestPanelProps) {
  ...
  const handleApplyUpgrade = (upgrade: NestUpgradeDefinition) => {
    if (confirmUpgrade !== upgrade.id) {
      ...
      setTimeout(() => setConfirmUpgrade(null), 3000);
      return;
    }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[#12121a]/50 rounded-xl border border-[#2a2a3a]/50 p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-medium text-[#e8e8ec] text-lg">Ninho</h3>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-[#a78bfa]">{comfortPercent}%</span>
          </div>
        ...
        {nestData.style !== 'basic' && (
          <p className="text-xs text-[#6a6a7a] mt-3">{styleDescription}</p>
        )}
      </div>
        ...
        {slotConfig.map(({ type, label, emoji }) => {
          ...
          return (
            ...
            >
              ...
              {slot ? (
                <div className="bg-[#1a1a24]/50 rounded-lg p-3 mb-3 border border-[#a78bfa]/30">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-sm font-medium text-[#c4b5fd]">{slot.name}</span>
                    <span className="text-xs px-2 py-0.5 rounded bg-[#a78bfa]/20 text-[#a78bfa]">
                      +{slot.comfortBonus}
                    </span>
                  </div>
                  <p className="text-xs text-[#a0a0b0] italic leading-relaxed">{slot.description}</p>
                </div>
              ) : (
                <p className="text-sm text-[#6a6a7a] italic mb-3 px-1">
              ...
              {upgradesForSlot.map((upgrade) => {
                ...
                if (isAlreadyApplied) {
                  return (
                    <div
                      key={upgrade.id}
                      className="rounded-lg p-3 bg-[#a78bfa]/5 border border-[#a78bfa]/20"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span>{upgrade.emoji}</span>
                          <span className="text-sm text-[#a78bfa]">{upgrade.name}</span>
                        </div>
                ...
                return (
                  <div
                    key={upgrade.id}
                    className="rounded-lg p-3 bg-[#1a1a24]/30 border border-[#2a2a3a]/30"
                  >
                    <div className="flex items-start gap-3 mb-3">
                      <span className="text-2xl flex-shrink-0">{upgrade.emoji}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-[#e8e8ec] mb-1">{upgrade.name}</p>
                        <p className="text-xs text-[#a0a0b0] leading-relaxed">{upgrade.description}</p>
                      </div>
                    </div>

                    {/* Cost */}
                    <div className="flex items-center gap-3 mb-3 flex-wrap">
                      <span className="text-xs text-[#6a6a7a]">Custo:</span>
                      {costEntries.map(([matId, amount]) => {
                        const def = MATERIAL_DEFINITIONS[matId as MaterialId];
                        const available = materials[matId as MaterialId] || 0;
                        const hasEnough = available >= (amount || 0);

                        return (
                          <div key={matId} className="flex items-center gap-1">
                            <span className="text-sm">{def?.emoji || '?'}</span>
                            <span className={`text-sm ${hasEnough ? 'text-[#e8e8ec]' : 'text-red-400'}`}>
                              {available}/{amount}
                            </span>
                          </div>
                        );
                    ...
                    <button
                      ...
                      `}
                    >
                      {isConfirming ? '✓ Confirmar Aplicação' : 'Aplicar'}
                    </button>

                    {/* Show reason if can't apply */}
                    {!checkResult.canApply && checkResult.reason && (
      ...
      <div className="bg-[#12121a]/50 rounded-xl border border-[#2a2a3a]/50 p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {(Object.entries(materials) as [MaterialId, number][]).map(([matId, qty]) => {
            ...
            return (
              <div
                key={matId}
                className={`
                  flex items-center gap-2 p-3 rounded-lg bg-[#1a1a24]/50
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-base font-bold text-[#a78bfa]">x{qty}</p>
                </div>
              </div>
            );
          })}
      ...
      <div className="bg-[#12121a]/30 rounded-xl border border-[#2a2a3a]/30 p-4">
        <p className="text-xs text-[#6a6a7a] text-center italic leading-relaxed">
